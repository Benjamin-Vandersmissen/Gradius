...
void resources::Bullet::loadFromIni(const std::string &path, const ini::Configuration &configuration) {
    ...
    m_type = configuration["Bullet"]["Type"].as_string_or_default(m_type);
    m_firedSoundBuffer = std::make_shared<sf::SoundBuffer>();
    m_firedSoundBuffer->loadFromFile(path + configuration["Bullet"]["FiredSound"].as_string_or_default("./sounds/ShotFired.wav"));
